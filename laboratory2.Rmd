...
However, our intention is to look into the details of the package. Therefore, it is much more appropiate to clone the repository into a folder and then, investigate the package implementation.

- _Clone the package and open a new RStudio window with the package project.  (You can use the terminal with "git clone" command, or the RStudio option within "New" option)._
...
Once we have reviewed the schema of the XML, it is time for some coding. It is specially important to restrict the scope of each one of the functions, avoiding functions that do too many things. [SOLID?]
